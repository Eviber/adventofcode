...
pub fn solve(input: &str) -> usize {
    ...
    updates
        ...
        .filter(|update| is_invalid(update, &rules))
        .map(|update| fix_update(update, &rules))
        .map(|update| update[update.len() / 2])
        .sum()
}

fn fix_update(mut update: Vec<usize>, rules: &HashMap<usize, Vec<usize>>) -> Vec<usize> {
...
fn is_invalid(update: &[usize], rules: &HashMap<usize, Vec<usize>>) -> bool {
    update.iter().enumerate().any(|(i, page)| {
        update
            .iter()
            .take(i)
            .any(|n| rules.get(page).map(|v| v.contains(n)).unwrap_or(false))
    })
}
